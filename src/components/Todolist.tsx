  ...
  const handleAddTask = (e: React.FormEvent) => {
    ...
    axios.post("http://localhost:8080/todoList", newTaskObj)
      ...
      .catch(error => {
        ...
        swal("Error", "There was an error adding the new task!", "error");
      });
  };
  ...
  const handleToggleTask = (id: number) => {
    ...
    axios.put(`http://localhost:8080/todoList/${id}`, updatedTask)
      ...
      .catch(error => {
        ...
        swal("Error", "There was an error updating the task!", "error");
      });
  };
      ...
      if (willDelete) {
        axios.delete(`http://localhost:8080/todoList/${id}`)
          ...
          .catch(error => {
            ...
            swal("Error", "There was an error deleting the task!", "error");
          });
      } else {
  ...
  const handleSaveEdit = (id: number) => {
    ...
    axios.put(`http://localhost:8080/todoList/${id}`, updatedTask)
      ...
      .catch(error => {
        ...
        swal("Error", "There was an error saving the edited task!", "error");
      });
  };
                          ...
                          {filteredTasks.map(task => (
                            ...
                            >
                              ...
                              <div style={{ display: "flex", gap: "20px" }}>
                                {editingTaskId === task.id ? (
                                  <button onClick={() => handleSaveEdit(task.id)} className="btn btn-success btn-sm" aria-label="Save edited task">
                                    Save
                                  </button>
                                ) : (
                                  <>
                                    <button
                                      onClick={() => handleEditTodo(task.id, task.name)}
